...
class _SearchScreenState extends State<SearchScreen> {
  ...
  Future<void> _performSearch(String query) async {
    ...
    try {
      if (_searchType == 0) {
        // Search Recipes
        final res = await http.get(Uri.parse('http://192.168.101.133:3000/api/recipes?search=$query&page=$_page&limit=$_limit'), headers: {'ngrok-skip-browser-warning': 'true'});
        ...
        final res = await http.get(
          Uri.parse('http://192.168.101.133:3000/api/users/search?q=$query'),
  ...
  Future<void> _fetchMore() async {
    ...
    _page++;
    try {
      final res = await http.get(Uri.parse('http://192.168.101.133:3000/api/recipes?search=$_searchQuery&page=$_page&limit=$_limit'), headers: {'ngrok-skip-browser-warning': 'true'});
      if (res.statusCode == 200) {
        final decoded = jsonDecode(res.body);
        if (decoded is List) {
          setState(() {
                    ...
                    return ListTile(
                      ...
                      leading: CircleAvatar(
                        ...
                        backgroundImage: user['profileImage'] != null 
                            ? NetworkImage('http://192.168.101.133:3000/uploads/${user['profileImage']}') 
                            : null,
